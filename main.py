...
# 下面的参数请直接修改default后运行源码
parser.add_argument('-d', '--dataset', type=str,
                    default='ferplus', choices=['mnist'])
...
parser.add_argument('--lr-cent', type=float, default=0.5,
                    help="learning rate for center loss")
parser.add_argument('--weight-cent', type=float, default=1,
...
def main():
    ...
    sys.stdout = Logger(
        osp.join(args.save_dir, 'log_' + args.dataset + '.txt'))
    ...
    # 下面数行代码是使用gpu训练后再到cpu上训练的无奈之举
    # emotion_dict = torch.load(
    #     "model.pth", map_location=torch.device('cpu')).eval().state_dict()
    ...
    criterion_cent = CenterLoss(
        num_classes=dataset.num_classes, feat_dim=8, use_gpu=use_gpu) #中心损失
    optimizer_model = torch.optim.Adam(
        model.parameters(), lr=args.lr_model, weight_decay=5e-04)
    optimizer_centloss = torch.optim.Adam(
        criterion_cent.parameters(), lr=args.lr_cent)

    if args.stepsize > 0:
        scheduler = lr_scheduler.StepLR(
            optimizer_model, step_size=args.stepsize, gamma=args.gamma)
...
def train(model, criterion_xent, criterion_cent,
    ...
    for batch_idx, (data, labels) in enumerate(trainloader):
        ...
        features, outputs = model(data)  # model输出xy
        loss_xent = criterion_xent(outputs, labels)  # y进交叉熵损失
        loss_cent = criterion_cent(features, labels)  # x进中心损失
    ...
    if args.plot:
        ...
        plot_features(all_features, all_labels,
                      num_classes, epoch, prefix='train')
    # 如需保存模型，则请解除下一行的注释
    # torch.save(model,'model123.pth')
...
def test(model, testloader, use_gpu, num_classes, epoch):
    ...
    if args.plot:
        ...
        all_labels = np.concatenate(all_labels, 0)
        plot_features(all_features, all_labels,
                      num_classes, epoch, prefix='test.py')
